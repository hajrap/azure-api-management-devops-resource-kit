    ...
    public class AzureKeyVaultHealthCheck : IHealthCheck
    {
        ...
        public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                foreach ((string key, bool checkExpired) in _options.Certificates)
                {
                    CertificateClient certificateClient = CreateCertificateClient();
                    Azure.Response<KeyVaultCertificateWithPolicy> certificate = await certificateClient.GetCertificateAsync(key, cancellationToken: cancellationToken);

                    if (checkExpired && certificate.Value.Properties.ExpiresOn.HasValue)
                    {
                        DateTimeOffset expirationDate = certificate.Value.Properties.ExpiresOn.Value;
