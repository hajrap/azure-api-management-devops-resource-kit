...
namespace com.dxc.lxp.Services.HealthChecks
{
    internal class ApplicationInsightsPublisher
        : IHealthCheckPublisher
    {
        private const string EVENT_NAME = "AzureFunctionHealthCheck";
        private const string METRIC_STATUS_NAME = "AzureFunctionHealthCheckStatus";
        private const string METRIC_DURATION_NAME = "AzureFunctionHealthCheckDuration";
        private const string HEALTHCHECK_NAME = "AzureFunctionHealthCheckName";
        ...
        public Task PublishAsync(HealthReport report, CancellationToken cancellationToken)
        {
            ...
            TelemetryClient client = GetOrCreateTelemetryClient();
        ...
        {
            foreach (KeyValuePair<string, HealthReportEntry> reportEntry in report.Entries.Where(entry => !_excludeHealthyReports || entry.Value.Status != HealthStatus.Healthy))
            {
            ...
            foreach (KeyValuePair<string, HealthReportEntry> reportEntry in report.Entries.Where(entry => entry.Value.Exception != null))
            {
            ...
            if (_client == null)
            {
                lock (sync_root)
                {
                    if (_client == null)
                    {
                        ...
                        TelemetryConfiguration configuration = string.IsNullOrWhiteSpace(_instrumentationKey)
